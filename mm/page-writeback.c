...
int test_clear_page_writeback(struct page *page)
{
	...
	//if(!PageLocked(page))
	//	printk("ksg: page is not locked in tcpw");
	//if (!mapping)
	//	printk("ksg: in test_clear_page_writeback, no mapping");
	//if (!mapping_use_writeback_tags(mapping))
	//	printk("ksg: in test_clear_page_writeback, no mapping_use_writeback_tags");
	if (mapping && mapping_use_writeback_tags(mapping)) {
		struct inode *inode = mapping->host;
		struct backing_dev_info *bdi = inode_to_bdi(inode);
		unsigned long flags;

		//printk("ksg: mapping && mapping_use_writeback_tags(mapping)");
