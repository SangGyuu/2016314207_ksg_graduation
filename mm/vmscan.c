...
/*static pageout_t ksg_pageout(struct page *page, struct address_space *mapping)
{
	if (clear_page_dirty_for_io(page)) {
		int res;
		struct writeback_control wbc = {
		...
		//printk("ksg: pageout, clear page dirty for io success");
		//ClearPageDirty(page);

		SetPageReclaim(page);
		res = mapping->a_ops->writepage(page, &wbc);
		//printk("ksg: res = %d", res);
		if (res < 0)
			handle_write_error(mapping, page, res);
		if (res == AOP_WRITEPAGE_ACTIVATE) {
			//printk("ksg: res is aop_writepage_activate");
			ClearPageReclaim(page);
			return PAGE_ACTIVATE;
		}
				...
				unsigned int classzone_idx)
{
	...
	if (!remaining &&
		...
		//cnt_page = background_swapout();
		printk("ksg: kswapdsleep\n");
		//cnt_page = 0;
...
/*
 * zram_to_nbd - find the page of target process that exist in ZRAM and then,
 * transfer that page to NBD
 */
int zram_to_nbd;
int zram_to_nbd_handler(struct ctl_table *table, int write,
						void __user *buffer, size_t *length, loff_t *ppos)
{
	...
	if(write)
	{
		...
		pid_t 					pid = (pid_t)zram_to_nbd;	// get target pid through sysctl
		...
		if(!page)
		{
			printk("ksg: can't allocate page\n");	// for check page alloc
			return 0;
		}
					...
					if(is_swap_pte(*pte))		// if the pte is swapped (swp_entry)
					{
						...
						else					// page is swapped and swap entry.
						{
							if(swp_type(entry) == ZRAM_TYPE)	// swap entry is in ZRAM (type == 0)
							{
								...
								{
									struct address_space *mapping;
									swp_entry_t nbd_entry = get_swap_page_of_type(NBD_TYPE);
									pte_t nbd_pte = swp_entry_to_pte(nbd_entry);
									//swp_entry_t zram_entry = get_swap_page_of_type(ZRAM_TYPE);
									//pte_t zram_pte = swp_entry_to_pte(zram_entry);
									int pageout_res;
									...
									/*test_page = find_get_page(swap_address_space(entry),
											swp_offset(entry));		// test if page has swapined
									if(test_page)
										printk("ksg: already in cache");*/						

									lock_page(page);
									add_to_swap_cache(page, nbd_entry, 
											__GFP_HIGH|__GFP_NOMEMALLOC|__GFP_NOWARN);
									set_page_private(page, nbd_entry.val);
									set_page_dirty(page);
									...
									pageout_res = write_one_page(page);
									//if(pageout_res != PAGE_SUCCESS)
									//	printk("ksg: pageout not success");

									__delete_from_swap_cache(page);

									//printk("ksg: addr %x from origin entry %x to zram %x", vpage, entry,new_entry);
									set_pte(pte, nbd_pte);
								}
							}
...
EXPORT_SYMBOL(zram_to_nbd_handler);

int foreground_uid;

int zram_to_nbd_swapout(struct task_struct *task)
{
		//pid_t 					pid = (pid_t)zram_to_nbd;	// get target pid through sysctl
		//struct task_struct 		*task = find_task_by_vpid(pid);	// get target_process' struct task_struct
		struct vm_area_struct 	*vma;
...
int background_swapout(void)
{
	int cnt_page = 0;
	struct task_struct *task;
	for_each_process(task) {
		if(task->cred->uid.val != (uid_t)foreground_uid) {
			printk("ksg: task->cred->uid = %d", (int)task->cred->uid.val);
			//cnt_page += zram_to_nbd_swapout(task);	
		}
	}
	return cnt_page;
}
...
int bulk_prefetcher_handler(struct ctl_table *table, int write,
							void __user *buffer, size_t *length, loff_t *ppos)
{
	int rc = proc_dointvec_minmax(table, write, buffer, length, ppos);
	if(rc)
	...
	if(write)
	{
		pid_t 				pid = (pid_t)bulk_prefetcher;	// get target pid through sysctl
...
retry:
	...
	printk("ksg: bulk prefetcher end");
	return 0;
}
EXPORT_SYMBOL(bulk_prefetcher_handler);

/* ksg end */


/* sysctl declare struct
int sysctl_name;
int sysctl_name_handler(struct ctl_table *table, int write,
						void __user *buffer, size_t *length, loff_t *ppos)
{
	int rc = proc_dointvec_minmax(table, write, buffer, length, ppos);
	if(rc)
